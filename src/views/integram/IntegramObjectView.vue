<template>
  <div class="integram-objects">
    <div class="page-header">
      <div class="header-left">
        <button class="back-btn" @click="$router.push(`/integram/${$route.params.database}/dict`)">
          <i class="pi pi-arrow-left"></i>
        </button>
        <h2><i class="pi pi-list"></i> {{ typeName || `Тип #${$route.params.typeId}` }}</h2>
      </div>
...
async function loadData() {
  ...
  try {
    ...
    if (response?.object && Array.isArray(response.object)) {
      ...
      // Extract columns from reqs metadata if not loaded from metadata endpoint
...
<style scoped>
.integram-objects { max-width: 1400px; margin: 0 auto; }
